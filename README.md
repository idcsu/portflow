...
PortFlow 是一个稳定优先的内部多节点 TCP/UDP 端口转发管理面板。覆盖完整中文 Web 管理、成员权限与操作审计、TOTP 二次验证、多节点 Agent、TCP/UDP 直连与基于既有 WireGuard 的双节点中转、限速、监控、部署诊断和升级回滚。
...
```bash
curl -fsSL https://raw.githubusercontent.com/idcsu/portflow/v1.1.2/install.sh \
  | sudo bash -s -- install --repo idcsu/portflow --version 1.1.2
```
...
首次打开 Web UI 时，系统会自动检查是否需要初始化管理员；只有数据库中没有任何账号时才显示初始化表单。初始化接口只能成功执行一次，密码至少需要 12 个字符。

登录后从右上角账号菜单进入“账户安全”，可以为每个账号开启标准 TOTP 二次验证。可使用任意兼容 TOTP 的手机验证器，开启时会生成 8 个只显示一次的一次性恢复码。TOTP 密钥使用 `PORTFLOW_MFA_ENCRYPTION_KEY` 进行 AES-256-GCM 加密后存入数据库；全新安装会生成独立随机密钥。从 v1.0.x 首次升级时，兼容层会使用安装器原先生成的稳定 64 位十六进制机密，避免旧管理器因不认识新配置而升级失败。不要手工更换最终写入的加密密钥，否则已有账号的二次验证密钥将无法解密。

## 当前 API

- `GET /api/v1/health`：控制面健康检查；
- `POST /api/v1/setup/admin`：首次创建管理员，仅可使用一次；
- `GET /api/v1/setup/status`：公开返回是否仍需要初始化，不泄露成员信息；
- `POST /api/v1/auth/login`、`POST /api/v1/auth/logout`：会话登录与退出；
- `GET /api/v1/auth/me`：读取当前用户；
- `POST /api/v1/auth/mfa/setup`、`/enable`、`/disable`：当前账号设置、确认或关闭 TOTP 二次验证；
- `GET /api/v1/users`、`POST /api/v1/users`：管理员查看和创建成员；
- `PUT /api/v1/users/{id}`、`DELETE /api/v1/users/{id}`：管理员修改权限、重置成员二次验证或删除成员；
...
```bash
curl -fsSL https://raw.githubusercontent.com/idcsu/portflow/v1.1.2/install.sh \
  | sudo bash -s -- agent --repo idcsu/portflow --version 1.1.2 \
...
- 管理员可以创建和删除内部账号、分配管理员或普通成员角色、禁用/启用账号、重置密码，并在成员丢失手机和恢复码时重置其二次验证；
- 普通成员可以查看节点、监控和实时连接，并管理单节点直连线路；节点注册、双节点中转、运行日志、操作审计和成员管理仅管理员可用；
- 角色、状态或密码变更会在同一存储操作中撤销目标账号全部旧会话，使权限立即生效；
- 当前管理员不能在成员列表中修改或删除自己，存储层还会原子阻止最后一名启用管理员被禁用、降级或删除；
