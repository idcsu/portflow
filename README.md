...
```bash
curl -fsSL https://raw.githubusercontent.com/idcsu/portflow/v1.0.2/install.sh \
  | sudo bash -s -- install --repo idcsu/portflow --version 1.0.2
```
...
Agent 仅允许远程控制地址使用 HTTPS；`http://127.0.0.1` 和 `http://localhost` 只用于本机开发。注册成功后，节点身份和凭证以 `0600` 权限原子写入本地配置，注册令牌立即失效，Agent 随即进入持续运行状态。
正式部署可使用 `-enrollment-token-file` 和 `-enroll-only`，避免一次性令牌出现在进程参数中，并在启动 systemd 前完成身份写入。
