...
```bash
curl -fsSL https://raw.githubusercontent.com/idcsu/portflow/v1.1.0/install.sh \
  | sudo bash -s -- install --repo idcsu/portflow --version 1.1.0
```
...
登录后从右上角账号菜单进入“账户安全”，可以为每个账号开启标准 TOTP 二次验证。可使用任意兼容 TOTP 的手机验证器，开启时会生成 8 个只显示一次的一次性恢复码。TOTP 密钥使用 `PORTFLOW_MFA_ENCRYPTION_KEY` 进行 AES-256-GCM 加密后存入数据库；一键安装和旧版本更新会自动生成并以 `0600` 权限保存该加密密钥。不要在后续更新中手工更换它，否则已有账号的二次验证密钥将无法解密。
...
```bash
curl -fsSL https://raw.githubusercontent.com/idcsu/portflow/v1.1.0/install.sh \
  | sudo bash -s -- agent --repo idcsu/portflow --version 1.1.0 \
