...
```bash
curl -fsSL https://raw.githubusercontent.com/OWNER/REPO/v1.0.2/install.sh \
  | sudo bash -s -- install --repo OWNER/REPO --version 1.0.2
```
...
```bash
curl -fL https://raw.githubusercontent.com/OWNER/REPO/v1.0.2/install.sh -o portflow-install.sh
less portflow-install.sh
sudo bash portflow-install.sh install --repo OWNER/REPO --version 1.0.2
```

也可以从已经下载的源码离线安装：

```bash
sudo bash install.sh install --source "$PWD" --version 1.0.2
```
